  ...
  ({ header, body, footer }, ref) => {
    ...
    useEffect(() => {
      ...
      const handleMouseDown = (event: MouseEvent) => {
        if (
          resizeD &&
          event.target === resizeDiv.current &&
          event.target !== headerRef
        ) {
      ...
      const handleMouseMove = (event: MouseEvent) => {
        return throttle(() => {
          ...
          if (resize) {
            ...
            // console.log("RESIZING", { mouseDelta: mouseDeltaX, initialWidth });
            const newWidth = Math.max(300, initialWidth + mouseDeltaX);
            const newHeight = Math.max(229, initialHeight + mouseDeltaY);

            // TODO: do not allow exit the viewport
            dialog.style.width = `${newWidth}px`;
            dialog.style.height = `${newHeight}px`;
            // console.log(dialog.style.width, "width");
            // console.log(dialog.style.height, "height");
          }
        }, 20)();
      };
      const handleMouseUp = () => {
      ...
      <>
        ...
        <div className={style.dialog}>
          <dialog ref={dialogWrapper}>
            ...
            <div>
              <div ref={resizeDiv} className={style.resize}>
                {" "}
              </div>
              <div ref={resizeDiv} className={style.resize}>
                {" "}
